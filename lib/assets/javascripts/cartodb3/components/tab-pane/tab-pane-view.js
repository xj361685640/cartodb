  ...
  _renderSelected: function (model, isSelected) {
    ...
    if (isSelected) {
      if (this._selectedView) {
        this._selectedView.$el.addClass('animation-testOut');
        
        // setTimeout(function(){
        //   self._selectedView.clean();
        //   self.removeView(self._selectedView);
        // },200);
      }
      setTimeout(function(){
        self._selectedView = self._renderTabPaneContentView(model);
        self._selectedView.$el.addClass('animation-test');
      },201 );
