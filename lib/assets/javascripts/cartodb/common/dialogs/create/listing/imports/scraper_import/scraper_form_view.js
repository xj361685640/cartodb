...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate(this.options.template || 'common/views/create/listing/import_types/scraper_form');
    this._initBinds();
  ...
  _onSubmitForm: function(e) {
    ...
    var iframe = $('#scraperIframe');
    iframe.attr('src', '/scraper?page=' + value);
    iframe.bind('load', function(e) {
      iframe.contents().find('table').bind('click',function(e) {
        var elementId = $(this).attr('id');

        // Change model
        //var importType = this.model.get('service_name') ? 'service' : 'url';
        this.model = new Upload2Model(null, { user: this.user });
        this.model.set({
          type: 'url',
          value: value + '?cdb_extract_css=' + elementId,
          service_item_id: value + '?cdb_extract_css=' + elementId,
          state: 'idle'
        });
