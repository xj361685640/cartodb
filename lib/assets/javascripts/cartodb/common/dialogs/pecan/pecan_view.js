...
module.exports = BaseDialog.extend({
  ...
  _onChangeColumns: function(column) {
    ...
    console.log(column.get("name") + ": " + this._getAnalyzedColumns().length + "/" + this.columns.length);
  ...
  _analyzeStats: function(name, stats) {
    ...
    if (!stats.count) {
      console.log(name + ' rejected because zero count', stats.count);
      return false;
    }

    if (stats.count > 3000) {
      console.log(name + ' rejected because more than 3000 rows', stats.count);
      return false;
    }

    if (stats.null_ratio > 75) {
      console.log(name + ' rejected because the null ratio > 75: ', stats.null_ratio);
      return false;
    }
    ...
    if (distinctPercentage > 75) {
      console.log(name + ' rejected because the distinctPercentage > 75: ', distinctPercentage);
      return false;
    }
  ...
  _addCard: function(url, response) {
    ...
    /*var view = ViewFactory.createByTemplate('common/dialogs/pecan/card', {
      column: response.column,
      wizard: wizardName,
      null_count: Utils.formatNumber(response.null_count),
      weight: response.stats.weight,
  ...
  _onLoadWizard: function() {
    ...
    if (wizard === 'choropleth') {
      ...
      console.log("applying: " + ramp +  " to " + property + " (" + dist + ")");
