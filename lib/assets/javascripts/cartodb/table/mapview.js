...
if (typeof(google) !== 'undefined') {
  ...
  cdb.admin.MapTab = cdb.core.View.extend({
    ...
    events: {
      ...
      'click .vector_processing.button':  'killEvent',
    },
  ...
  setupMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      this._addLegends();
      this._addOverlays();
      this._addVectorProcessingDropdown();
  ...
  _addVectorProcessingDropdown: function() {

    if (!this.vectorProcessingDropdown) {

      this.vectorProcessingDropdown = new cdb.admin.VectorProcessingDropdown({
        vis: this.master_vis,
        canvas: this.canvas,
        mapView: this.mapView,
        target: $('.vector_processing'),
        position: "position",
        collection: this.vis.overlays,
        template_base: "table/views/widget_dropdown",
        tick: "left",
        horizontal_position: "left",
        horizontal_offset: "318",
        vertical_offset: "300"
      });

      this.addView(this.vectorProcessingDropdown);

      this.vectorProcessingDropdown.bind("onVectorProcessingDropdownOpen", function(){
        this.slidesPanel && this.slidesPanel.hide();
      }, this);


      cdb.god.bind("closeDialogs", this.vectorProcessingDropdown.hide, this.vectorProcessingDropdown);
      cdb.god.bind("closeVectorProcessingDropdown", this.vectorProcessingDropdown.hide, this.vectorProcessingDropdown);

      $(".vector_processing").append(this.vectorProcessingDropdown.render().el);
    }
