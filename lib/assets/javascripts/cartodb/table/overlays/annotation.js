cdb.admin.overlays.Annotation = cdb.geo.ui.Annotation.extend({
  ...
  events: {
    ...
    'keyup .js-text':     '_onKeyUp',
    'paste .js-text':     '_onPaste'
  },
  ...
  _onKeyUp: function(e) {
    ...
    this.model.set({ text: this.$(".js-text").val() }, { silent: true });
  },
  ...
  _applyStyle: function() {
    ...
    this._toggleBorderStyle();
    this._toggleWhiteBox();

    this.$(".content").css("padding", boxPadding);

    this.$(".js-font-style").css(style);
    this.$el.css("z-index", style["z-index"]);

    this.$(".stick").css({ width: lineWidth, left: -lineWidth });

    this._removeFontStyle();

    var fontFamilyClass = this._getFontFamilyClass(fontFamily);
  ...
  _removeFontStyle: function() {
    this.$el.removeClass(_.map(this._FONTS, function(font) { return font; }).join(" "));
  },

  _getFontFamilyClass: function(fontFamily) {
    return this._FONTS[fontFamily];
  },

  _toggleBorderStyle: function() {
    var className = 'border-dark';
    var boxOpacity = this.model.get('style')['box-opacity'];

    this.$el[boxOpacity === 0 ? 'addClass' : 'removeClass'](className);
  },

  _toggleWhiteBox: function() {
    var className = 'white-box';
    var boxColor  = this.model.get('style')['box-color'];

    this.$el[boxColor === '#FFFFFF' ? 'addClass' : 'removeClass'](className);
  },
  ...
  _enableEditingMode: function() {
    ...
    var fontFamilyName = style["font-family-name"];
    ...
    this.$(".js-text").focus();
    this.$(".js-text").val(text);

    this.$(".js-hint").fadeIn(150);
  },
  ...
  _disableEditingMode: function() {
    ...
    if (text) {
      ...
      self.$(".js-hint").fadeOut(150, function() {
        self.$el
