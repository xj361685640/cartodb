...
module.exports = CoreView.extend({
  ...
  _getIcons: function () {
    ...
    return this._icons.map(function (icon) {
      ...
      return {
        selected: icon.get('selected') || icon.getURLFor(icon.get('icon')) === selectedIcon.image, // TODO: change to `url` instead of image,
        icon: icon,
        type: type,
        name: icon.get('name'),
  ...
  _changeIcon: function (m) {
    this.trigger('change', {
      image: m.get('icon').get('public_url'),
      kind: m.get('icon').get('kind')
    }, this);
  },
