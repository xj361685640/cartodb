...
var createFakeDashboard = function (layers) {
  ...
  fakeMap.latLngToPixel = () => {
    return { x: 100, y: 20 };
  };

  fakeMap.getMapViewSize = () => {
    return { x: 100, y: 100 };
  };

  var fakeVis = new Backbone.Model();
  fakeVis.map = fakeMap;
  fakeVis.getStaticImageURL = jest.createSpy('getStaticImageURL');
...
describe('deep-insights-integrations/dii', () => {
  var el;
  var dashBoard;
  ...
  describe('time series', () => {
    var xhrSpy = jest.createSpyObj('xhr', ['abort', 'always', 'fail']);
    ...
    it('should create time-series widget on layer changes', () => {
    ...
    it('should create only one time-series widget', () => {
      spyOn(this.integrations, '_createTimeseries').and.callThrough();
      this.layerDefModel.styleModel.set({animated: animatedChanged1});
      this.layerDefModel.set({alias: 'wadus'});
  ...
  describe('_resetStylesIfNoneApplied', () => {
    beforeEach(() => {
      this.layerDefModel = new LayerDefinitionModel({
        id: 'harr',
        kind: 'carto',
    ...
    it('should not reset styles if layer doesn\'t have none styles', () => {
    ...
    it('should not reset styles if node definition has not finished', () => {
      var nodeMod = this.analysis._analysisCollection.at(1);
    ...
    it('should not reset styles if node type is source', () => {
      var nodeMod = this.analysis._analysisCollection.at(1);
    ...
    it('should fetch geometry if it is not defined until reset styles', () => {
      var nodeDef = this.layerDefModel.getAnalysisDefinitionNodeModel();
      var nodeMod = this.analysis._analysisCollection.at(1);
      nodeDef.queryGeometryModel.set('simple_geom', '', { silent: true });
      nodeMod.isDone.and.returnValue(true);
      spyOn(nodeDef.queryGeometryModel, 'fetch').and.callFake(() => {
    ...
    it('should reset styles if layer has none styles', () => {
      var nodeMod = this.analysis._analysisCollection.at(1);
      nodeMod.isDone.and.returnValue(true);
  ...
  describe('when a widget-definition is created', () => {
    beforeEach(() => {
    ...
    it('should bind widgets changes', () => {
      expect(this.integrations._bindWidgetChanges).toHaveBeenCalled();
    });

    it('should call widgets service properly', () => {
    ...
    it('should create the corresponding widget model for the dashboard', () => {
      expect(dashBoard.createFormulaWidget).toHaveBeenCalled();

      var args = dashBoard.createFormulaWidget.calls.argsFor(0);
      expect(args[0]).toEqual(jest.objectContaining({
        title: 'avg of something',
        layer_id: 'l-1',
        column: 'col',
        operation: 'avg',
    ...
    it('should enable show_stats and show_options for the created widget model', () => {
    ...
    describe('when definition changes data', () => {
      ...
      describe('of the source', () => {
        ...
        it('should maintain normal params but massage the source', () => {
          expect(this.widgetModel.update).toHaveBeenCalled();
    ...
    describe('when definition changes type', () => {
      beforeEach(() => {
        this.widgetModel = dashBoard.getWidget(this.model.id);
        spyOn(this.widgetModel, 'remove').and.callThrough();
      ...
      it('should remove the corresponding widget model', () => {
        expect(this.widgetModel.remove).toHaveBeenCalled();
      });

      describe('should create a new widget model for the type', () => {
        beforeEach(() => {
          ...
          expect(dashBoard.createCategoryWidget).toHaveBeenCalledWith(jest.any(Object), jest.any(Object));
        });
        ...
        it('with prev layer-defintion', () => {
          expect(dashBoard.createCategoryWidget.calls.argsFor(0)[1].id).toEqual('l-1');
        });
      });

      it('should set show_stats in the new widget model', () => {
        var widgetModel = dashBoard.getWidget(this.model.id);
        expect(widgetModel.get('show_stats')).toBeTruthy();
      });
    });
  ...
  describe('when a new layer definition model is created', () => {
    beforeEach(() => {
      ...
      this.cdbjsLayer.update = jest.createSpy('update');
      this.cdbjsLayer.remove = jest.createSpy('remove');
    ...
    it('should create the CartoDB.js layer at the given position (order)', () => {
      this.layerDefinitionModel = this.layerDefinitionsCollection.add({
    ...
    it('should update the CartoDB.js layer at the right position', () => {
    ...
    describe('when the layer definition model is updated', () => {
      beforeEach(() => {
      ...
      it('should update the CartoDB.js layer', () => {
    ...
    describe('when layer type is changed to torque', () => {
      beforeEach(() => {
        this.layerDefinitionModel.set('type', 'torque');
      });

      it('should have re-created the layer', () => {
      ...
      xit("should have created a timeslider widget if there wasn't one", () => {
        expect(this.widgetDefinitionsCollection.where({'type': 'time-series'}).length).not.toBeLessThan(1);
      });
    });

    describe('when layer has a source attribute here and not in CartoDB.js', () => {
      it('should set/update the source attribute', () => {
    ...
    describe('when removing layer', () => {
      beforeEach(() => {
        this.layerDefinitionsCollection.remove(this.layerDefinitionModel);
      });

      it('cartodb.js layer should be removed too', () => {
        expect(this.cdbjsLayer.remove).toHaveBeenCalled();
      });
    });
  });

  describe('when the base layer has changed', () => {
    beforeEach(() => {
    ...
    it('should re-create the cdb.js layer if type has changed', () => {
    ...
    it('should update the cdb.js layer if type has NOT changed', () => {
      this.layerDefinitionsCollection.at(0).set({
    ...
    it('should change the map provider', () => {
      this.layerDefinitionsCollection.at(0).attributes = _.pick(this.layerDefinitionsCollection.attributes, 'type');
      this.layerDefinitionsCollection.at(0).set({
        name: 'GMaps Hybrid',
        maxZoom: 40,
    ...
    describe('if new new base layer has labels', () => {
      beforeEach(() => {
      ...
      describe('if cdb.js has a layer with labels', () => {
        beforeEach(() => {
          this.cdbjsLabelsLayer = new Backbone.Model({ type: 'Tiled' });
          this.cdbjsLabelsLayer.update = jest.createSpy('update');
          this.cdbjsLabelsLayer.remove = jest.createSpy('remove');
        ...
        it('should update the cdb.js labels layer when something changes', () => {
      ...
      describe('if cdb.js does NOT have a layer with labels', () => {
        beforeEach(() => {
          this.integrations._getLayers.and.returnValue(new Backbone.Collection([
            this.cdbjsLayer
          ]));
        ...
        it('should create the cdb.js labels layer', () => {
          this.layerDefinitionsCollection.trigger('baseLayerChanged');
    ...
    describe('if new new base layer does NOT have labels', () => {
      beforeEach(() => {
        this.layerDefinitionsCollection.resetByLayersData([
          {
            'id': 'baseLayer',
      ...
      describe('if cdb.js has a layer with labels', () => {
        ...
        it('should remove the cdb.js labels layer', () => {
          this.layerDefinitionsCollection.trigger('baseLayerChanged');
  ...
  describe('scrollwheel', () => {
    it('when activating scrollwheel', () => {
  ...
  describe('when the style is changed', () => {
    it("should disable any activated widgets' autoStyle", () => {
  ...
  describe('autoStyle', () => {
    ...
    it('should cancel autostyle on remove widget', () => {
    ...
    it('should update layer definition model\'s autostyle properly', () => {
      var model = dashBoard.getWidget(category.id);
      model.set({autoStyle: true});
    ...
    it('should update layer definition model\'s previousCartoCSS only if not autoStyle applied', () => {
      ...
      layerDefinitionModel.styleModel.setPropertiesFromAutoStyle = () => {};
      ...
      expect(layerDefinitionModel.set).toHaveBeenCalledWith(jest.objectContaining({
      ...
      expect(layerDefinitionModel.set).not.toHaveBeenCalledWith(jest.objectContaining({
        previousCartoCSS: '#dummy {}'
      }));
    });

    it('should update layer definition model\'s style properly based on previous custom style', () => {
    ...
    it('should update layer definition model\'s color properly', () => {
      var model = dashBoard.getWidget(category.id);
    ...
    it('should disable autoStyle if aggregation is not simple', () => {
      var model = dashBoard.getWidget(category.id);
      model.set({autoStyle: true});
      expect(layerDefinitionModel.get('autoStyle')).toBe(model.id);
      expect(layerDefinitionModel.styleModel.setPropertiesFromAutoStyle).toHaveBeenCalled();
  ...
  describe('when analysis-definition-node is created', () => {
    beforeEach(() => {
      this.a0 = this.analysisDefinitionNodesCollection.add({
        id: 'a0',
        type: 'source',
    ...
    describe('when changed', () => {
      ...
      it('should analyse node again but with changed query', () => {
    ...
    describe('when an analysis-definition is added for source node', () => {
      ...
      describe('when analysis node has finished executing', () => {
        beforeEach(() => {
          this.analysis.findNodeById('a0').set('status', 'ready');
        });

        it('should not affect the query-schema-model if its a source', () => {
          expect(this.a0.querySchemaModel.set).not.toHaveBeenCalled();
        });
      });

      describe('when analysis-definition-node is removed', () => {
        beforeEach(() => {
          expect(this.analysis.findNodeById('a0')).toBeDefined();
    ...
    describe('when an analysis definition is added for non-source node', () => {
      beforeEach(() => {
      ...
      describe('when analysis node has finished executing', () => {
        ...
        it('should launch the onboarding analysis if the user saved the node', () => {
      ...
      describe('when analysis node has finished executing', () => {
        ...
        it('should update the sub-models', () => {
    ...
    describe('queryRowsCollection', () => {
      ...
      it('should reload map when removing a row', () => {
  ...
  describe('when a layer is moved', () => {
    it('should invoke moveCartoDBLayer function in CartoDB.js', () => {
      spyOn(this.integrations.visMap(), 'moveCartoDBLayer');
  ...
  it('should set converters when basemap changes', () => {
  ...
  describe('visMetadata', () => {
    ...
    it('update vis metadata when vis reload', () => {
      this.integrations._vis().trigger('reload');
      expect(this.mapDefinitionModel.setImageExportMetadata).toHaveBeenCalled();
      expect(this.mapDefinitionModel.setStaticImageURLTemplate).toHaveBeenCalled();
    });
  ...
  describe('.infowindow', () => {
    beforeEach(() => {
      ...
      this.cdbLayer.infowindow = jest.createSpyObj('infowindow', ['update']);
    ...
    describe('w/o fields', () => {
      ...
      describe('when template is changed', () => {
        beforeEach(() => {
        ...
        xit('should set a "none" template', () => {
          expect(this.cdbLayer.infowindow.update).toHaveBeenCalledWith({
            alternative_names: {},
            autoPan: true,
            content: '',
    ...
    describe('w/ fields', () => {
      ...
      describe('when both template and fields are changed', () => {
        beforeEach(() => {
          this.layerDefinitionsCollection.at(0).infowindowModel.set({
            'fields': [
              {
        ...
        it('should update fields and template', () => {
          expect(this.cdbLayer.infowindow.update).toHaveBeenCalledWith({
            alternative_names: {},
            autoPan: true,
            content: '',
  ...
  describe('"syncing" of errors coming from cartodb.js models', () => {
    beforeEach(() => {
    ...
    it('should set turbo-carto errors on the layerDefinitionModel if CartoDB.js layer had an error', () => {
    ...
    it('should set turbo-carto errors on the layerDefinitionModel if CartoDB.js layer gets new errors', () => {
    ...
    it('should add an error in the notifier with the same id as the layer', () => {
  ...
  describe('stateDefinitionModel', () => {
    beforeEach(() => {
      // All "widgets dataviews" fetched, ready to listen state changes
      this.integrations._diDashboard._dashboard.vis.trigger('dataviewsFetched');
    });

    it('should be bind to state changes', () => {
      expect(this.integrations._diDashboard.onStateChanged).toHaveBeenCalled();
    });

    it('should change state model when there is any state map change from DI', () => {
      expect(this.integrations._diDashboard.onStateChanged).toHaveBeenCalled();
      this.integrations.visMap().set('center', [10, 20]);
      expect(this.stateDefinitionModel.updateState).toHaveBeenCalled();
    });

    it('should change state model when there is any state widget change from DI', () => {
      expect(this.integrations._diDashboard.onStateChanged).toHaveBeenCalled();
    ...
    it('should call to onBoundsSet when state triggers a "boundsSet" event', () => {
      var bounds = [ 808 ];
  ...
  describe('overlaysCollection', () => {
    it('should add overlay to CDB overlays collection when a new one is created', () => {
    ...
    it('should remove overlay from CDB overlays collection when one is removed', () => {
      expect(this.integrations._vis().overlaysCollection.size()).toBe(1);
      var overlayModel = new Backbone.Model({ id: 'hello', type: 'search' });
      this.integrations._overlayDefinitionsCollection.add(overlayModel);
      expect(this.integrations._vis().overlaysCollection.size()).toBe(2);
  ...
  describe('max/min zoom changes', () => {
    ...
    it('should change max and min zoom of the map if changes in map-definition-model', () => {
      expect(this.integrations.visMap().get('minZoom')).toBe(0);
      expect(this.integrations.visMap().get('maxZoom')).toBe(15);
    });

    it('should change map zoom if maxZoom is not as high as the current one', () => {
  ...
  describe('legends', () => {
    beforeEach(() => {
      ...
      this.bubble = jest.createSpyObj('bubble', ['show', 'hide', 'set', 'reset']);
      this.choropleth = jest.createSpyObj('choropleth', ['show', 'hide', 'set', 'reset']);
    ...
    it('should hide legend when a legend def model deleted', () => {
    ...
    it('should update legend when a legend def model update', () => {
      var layerDefModel = this.layerDefinitionsCollection.at(0);
      var legendDedfModel = this.legendDefinitionsCollection.findByLayerDefModelAndType(layerDefModel, 'choropleth');
