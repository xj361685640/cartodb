...
describe('data/wms-service', () => {
  beforeEach(() => {
    this.wmsService = new WMSService();
  });

  it('should add necessary params', () => {
    var url = 'http://myURL';
  ...
  it("shouldn't remove extra params in the URL", () => {
  ...
  it('should set the supported matrix sets to create a WMTS resource', () => {
    var url = 'http://myURL';
    this.wmsService.setUrl(url);
  ...
  describe('.supportedMatrixSets', () => {
    it('should return a subset of the supported matrix sets (used for a WMTS result)', () => {
      expect(this.wmsService.supportedMatrixSets(['foo', 'bar', 'EPSG:4258', 'baz', 'EPSG:4326'])).toEqual(['EPSG:4258', 'EPSG:4326']);
    });

    it('should return empty results for empty or missing input', () => {
