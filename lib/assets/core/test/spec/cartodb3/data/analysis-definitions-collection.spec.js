...
describe('cartodb3/data/analysis-definitions-collection', () => {
  beforeEach(() => {
  ...
  describe('.findAnalysisThatContainsNode', () => {
    describe('when given a node that does not have any analysis', () => {
      it('should return undefined', () => {
        expect(this.collection.findAnalysisThatContainsNode({id: 'x123'})).toBeUndefined();
      });
    });

    describe('when given a node is root', () => {
      it('should return the analysis', () => {
        expect(this.collection.findAnalysisThatContainsNode({id: 'c1'})).toBeDefined();
      });
    });

    describe('when given a node is contained in an analysis', () => {
      it('should return the analysis', () => {
  ...
  describe('.saveAnalysisForLayer', () => {
    beforeEach(() => {
      ...
      this.beforeSaveSpy = jest.createSpy('beforeSave');
      spyOn(this.A, 'save');
      spyOn(this.B, 'save');
      spyOn(this.C, 'save');
    ...
    describe('when given a layer with an already persisted analysis', () => {
      it('should save all analysis containing node', () => {
    ...
    describe('when given a layer has a source that is owned by another layer', () => {
      describe('when has a persisted analysis', () => {
        beforeEach(() => {
          this.layerB.set('source', 'a0');
          this.collection.saveAnalysisForLayer(this.layerB);
        });

        it('should destroy analysis that existed for layer', () => {
          expect(this.collection.pluck('node_id')).toEqual(['a1', 'c1']);
        });
      });

      describe('when has no persisted analysis', () => {
        beforeEach(() => {
          this.layerX.set('source', 'a0');
          this.collection.saveAnalysisForLayer(this.layerX);
        });

        it('should not create any analysis since its already persisted for its owning layer', () => {
          expect(this.collection.pluck('node_id')).toEqual(['a1', 'b1', 'c1']);
        });
      });
    });

    describe('when a node is added on a node', () => {
      beforeEach(() => {
  ...
  describe('.newAnalysisForNode', () => {
    ...
    it('should point to the given node id', () => {
