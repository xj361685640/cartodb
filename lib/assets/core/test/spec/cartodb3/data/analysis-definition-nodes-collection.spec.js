...
describe('data/analysis-definition-nodes-collection', () => {
  beforeEach(() => {
  ...
  describe('.createSourceNode', () => {
    describe('when given faulty input', () => {
      it('should throw an error', () => {
        var c = this.collection;
        expect(() => { c.createSourceNode({}); }).toThrowError(/required/);
        expect(() => { c.createSourceNode({id: 'missing table'}); }).toThrowError(/required/);
        expect(() => { c.createSourceNode({tableName: 'missing id'}); }).toThrowError(/required/);
        expect(() => { c.createSourceNode(); }).toThrowError(/required/);
      });
    });

    describe('when given valid input', () => {
      it('should add a new source node', () => {
      ...
      it('should allow to create source node with custom query', () => {
  ...
  describe('when sources are added', () => {
    beforeEach(() => {
      this.collection.add([{
        id: 'a0',
        type: 'source',
    ...
    it('should have created a model for each item', () => {
      expect(this.collection.pluck('id')).toEqual(['a0', 'a1', 'b0', 'b1']);
    });

    describe('when adding an analysis that contains references to existing sources', () => {
      beforeEach(() => {
        this.collection.add({
          id: 'd1',
          type: 'intersection',
      ...
      it('should have created a model for the new analysis node and source nodes if not already existing', () => {
        expect(this.collection.pluck('id')).toEqual(['a0', 'a1', 'b0', 'b1', 'c0', 'd1']);
      });
    });

    describe('a table model should be created', () => {
      it('from relatedTableData if available', () => {
      ...
      it('as a default model if relatedTableData is not available', () => {
    ...
    describe('._getNodeOnTop', () => {
      it('should return node on top', () => {
        expect(this.collection._getNodeOnTop()).toBe(this.b1);
      });
    });

    describe('._getTopNodeIndex', () => {
      it('should return node on top index', () => {
        expect(this.collection._getTopNodeIndex()).toEqual(3);
      });
    });

    describe('._isNodeOnTop', () => {
      it('should return true if given node is on top', () => {
        expect(this.collection._isNodeOnTop(this.b0)).toBe(false);
        expect(this.collection._isNodeOnTop(this.b1)).toBe(true);
      });
    });

    describe('._onQueryObjectsUpdated', () => {
      it('should fetch Query Objects if node is on top', () => {
