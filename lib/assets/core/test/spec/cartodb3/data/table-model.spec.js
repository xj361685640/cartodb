...
describe('data/table-model', () => {
  ...
  it('should provide means for a custom URL for columns collection', () => {
    expect(this.model.columnsCollection.url()).toEqual('/u/pepe/api/v1/tables/foobar_table/columns');
  });

  describe('getGeometryType', () => {
    it('should return the geometries those are present in the table', () => {
    ...
    it('shouldn\'t return the same geometry several times', () => {
  ...
  describe('.parse', () => {
    it('should take table_visualization attributes from response if they exist, not removing or overwritting geometry_types', () => {
    ...
    describe('synchronization', () => {
      it('should create synchronization model if sync data is provided and model is not created', () => {
      ...
      it('should update synchronization model if sync data is provided and that model is already created', () => {
        var syncTableModel = createTableModel({ synchronization: {} }, true);
    ...
    describe('permission', () => {
      it('should create permission model if permission attribute is provided', () => {
      ...
      it('should update permission model if permission attribute is provided and that model is already created', () => {
        var permissionTableModel = createTableModel({ permission: {} }, true);
  ...
  describe('.isSync', () => {
    it('should be truthy if there is a synchronization model and it has id', () => {
      var syncTableModel = createTableModel({ synchronization: { id: 'ha' } }, true);
      expect(syncTableModel.isSync()).toBeTruthy();
    });

    it('should be falsy if there isn\'t a synchronization model or if it doesn\'t have an id', () => {
  ...
  describe('getOwnerName', () => {
    it('should take the owner name from the table name if it is included', () => {
      this.model.set('name', 'perico.foobar_table');
      expect(this.model.getOwnerName()).toBe('perico');
    });

    it('should take the owner name from the permission if it is available and it is not included in the table name', () => {
  ...
  describe('.isReadOnly', () => {
    ...
    it('should be false if user does have write permissions', () => {
      var permissionTableModel = createTableModel({ permission: {} }, true);
