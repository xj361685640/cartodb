...
describe('dataset/dataset-content-view', () => {
  beforeEach(() => {
  ...
  describe('render', () => {
    ...
    it('should render the table as disabled if table belongs to other user', () => {
    ...
    it('should render the table as disabled if it is sync', () => {
    ...
    it('should not render sync info if table doesn\'t belong to the user', () => {
      this.tableModel.isSync.and.returnValue(true);
      spyOn(this.tableModel._permissionModel, 'isOwner').and.returnValue(false);
      this.view.render();
      expect(this.view.$('.Table').hasClass('is-disabled')).toBeTruthy();
  ...
  it('should set new table-name in table-view columnsCollection when it changes', () => {
