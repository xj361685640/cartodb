...
describe('cartodb3/editor/layers/layer-content-view/analyses/analysis-forms-collection', () => {
  beforeEach(() => {
    var configModel = new Backbone.Model();
    configModel.dataServiceEnabled = () => {
      return true;
    };
  ...
  describe('.resetByLayerDefinition', () => {
    beforeEach(() => {
      this.collection.resetByLayerDefinition();
    });

    it('should have created a form model for each node (excluding the source)', () => {
      expect(this.collection.pluck('id')).toEqual(['a2', 'a1']);
    });

    it('should have same attributes as the node models they represent', () => {
      expect(this.collection.get('a1').attributes).toEqual(jest.objectContaining(this.a1.attributes));
      expect(this.collection.get('a2').attributes).toEqual(jest.objectContaining(this.a2.attributes));
    });

    it('should mark them as persisted', () => {
      expect(this.collection.get('a1').attributes).toEqual(jest.objectContaining({persisted: true}));
      expect(this.collection.get('a2').attributes).toEqual(jest.objectContaining({persisted: true}));
    });
  });
  ...
  describe('.addHead', () => {
    ...
    it('should return the created formModel', () => {
      expect(this.res).toBeDefined();
    });

    it('should replace any existing node with same id', () => {
    ...
    it('should prepend new form model', () => {
      expect(this.collection.first().id).toEqual('a3');
    });
  });

  describe('.deleteNode', () => {
    describe('when given id represents an persisted node', () => {
      beforeEach(() => {
        spyOn(this.userActions, 'deleteAnalysisNode');
        this.collection.deleteNode('a2');
      });

      it('should destroy the node-definition for given id', () => {
        expect(this.userActions.deleteAnalysisNode).toHaveBeenCalledWith('a2');
      });
    });

    describe('when given id is for a new form model', () => {
      beforeEach(() => {
      ...
      it('should just remove the form model from the collection', () => {
        expect(this.collection.pluck('id')).toEqual([]);
      });
    });
  });

  describe('for a form model', () => {
    beforeEach(() => {
