...
describe('editor/layers/layer-content-view/analyses/analysis-controls-view', () => {
  beforeEach(() => {
    ...
    this.analysisNode.hasFailed = () => {
      return this.analysisNode.get('status') === 'failed';
    };

    this.analysisNode.isDone = () => {
      return this.analysisNode.get('status') === 'ready';
    };

    this.analysis = jest.createSpyObj('vis.analysis', ['findNodeById']);
  ...
  it('should not fetch quota if no analysis node', () => {
  ...
  describe('common', () => {
    ...
    it('should generate a viewModel', () => {
    ...
    it('should render when form changes', () => {
  ...
  describe('analysis without quota', () => {
    beforeEach(() => {
    ...
    describe('when form is valid', () => {
      ...
      it('should enable button when analysis node\'s status is ready', () => {
        expect(this.view.$el.html()).toContain('is-disabled');
    ...
    describe('when the analysis failed', () => {
      beforeEach(() => {
        this.formModel.set('foo', 'just to trigger re-render');
      });
    ...
    describe('when model is persisted', () => {
      beforeEach(() => {
        this.formModel.set('persisted', true);
        this.view.render();
      });

      it('should change label to apply', () => {
        expect(this.view.$('.js-save').html()).toContain('apply');
      });
    });
  });

  describe('analysis with quota', () => {
    beforeEach(() => {
    ...
    it('should not fetch quota when form changes and not valid', () => {
    ...
    it('should not let user run the analysis if form is not valid', () => {
      this.formModel.isValid.and.returnValue(false);
    ...
    it('should not let user run the analysis if user doesn\'t have credit assigned', () => {
    ...
    it('should not let user run the analysis if not enough quota', () => {
      spyOn(this.view, '_canSave').and.returnValue(true);
    ...
    it('should let user run the analysis if no credits but soft limit', () => {
      spyOn(this.view, '_canSave').and.returnValue(true);
    ...
    it('should get back to analyses list when cancel is clicked', () => {
      spyOn(this.view, '_canSave').and.returnValue(true);
    ...
    it('should change label to confirm', () => {
      spyOn(this.view, '_canSave').and.returnValue(true);
      mockSQL.mock(AnalysesQuotaEnough, 'success', {
        rows: [{'cdb_enough_quota': true}]
      });
    ...
    it('render a save button disabled after saved an analysis', () => {
    ...
    describe('dataservices api not existing', () => {
      beforeEach(() => {
        this.quotaInfo._state = 'error';
      });

      it('should avoid fetching quota', () => {
        spyOn(this.view, '_fetchQuota');
        expect(this.view._fetchQuota).not.toHaveBeenCalled();
      });

      it('should show an infowindow error if quota view should be shown for edited analysis', () => {
        spyOn(this.view, '_canSave').and.returnValue(true);
        this.view.render();
  ...
  describe('change analysis type', () => {
    ...
    it('should render properly if type changes', () => {
      spyOn(this.view, '_canSave').and.returnValue(true);
      this.view.render();
