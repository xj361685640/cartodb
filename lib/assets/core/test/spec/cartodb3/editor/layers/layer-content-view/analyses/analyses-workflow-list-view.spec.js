...
describe('editor/layers/layer-content-view/analyses/analyses-workflow-list-view', () => {
  beforeEach(() => {
  ...
  describe('when another non-selected node is clicked', () => {
    beforeEach(() => {
      this.view.$('li:eq(3)').click();
    });

    it('should change selected workflow item', () => {
      expect(this.viewModel.get('selectedNodeId')).toEqual('a1');
    });

    it('should only have one item selected still', () => {
      expect(this.view.$('.is-selected').length).toEqual(1);
    });
  });
});
