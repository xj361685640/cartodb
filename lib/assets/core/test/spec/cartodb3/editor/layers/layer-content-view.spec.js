...
describe('editor/layers/layer-content-view/lcv', () => {
  ...
  beforeEach(() => {
    ...
    this.widgetDefinitionsCollection.isThereTimeSeries = () => {
      return false;
    };
    this.widgetDefinitionsCollection.isThereOtherWidgets = () => {
      return false;
    };
  ...
  it('should go to prev stack layout step if arrow is clicked', () => {
    this.view.$('.js-back').click();
    expect(this.stackLayoutModel.prevStep).toHaveBeenCalledWith('layers');
  });

  describe('._renderContextButtons', () => {
    ...
    it('should render context buttons properly', () => {
    ...
    it('should render geometry buttons properly', () => {
      expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem').length).toBe(3);
    });

    it('should disable edit geometry buttons when node is read only and visible', () => {
    ...
    it('should disable edit geometry buttons when node is read only and not visible', () => {
      analysisDefNodeModel.isReadOnly.and.returnValue(true);
      layerDefinitionModel.set('visible', false);
      expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem.is-disabled').length).toBe(3);
    });

    it('should disable edit geometry buttons when node is neither visible nor read only', () => {
      analysisDefNodeModel.isReadOnly.and.returnValue(false);
      layerDefinitionModel.set('visible', false);
      expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem.is-disabled').length).toBe(3);
    });

    it('should not disable edit geometry buttons when node is visible and not read only', () => {
      analysisDefNodeModel.isReadOnly.and.returnValue(false);
      layerDefinitionModel.set('visible', true);
      expect($(dashboardCanvasEl).find('.Editor-contextSwitcherItem.js-newGeometryItem.is-disabled').length).toBe(0);
    });

    it('should add tipsy subview when not visible or read only', () => {
    ...
    it('should not add tipsy subview when not visible or read only', () => {
      this.view.clearSubViews();
      analysisDefNodeModel.isReadOnly.and.returnValue(false);
      layerDefinitionModel.set('visible', true);
    ...
    it('should display only one button per geometry', () => {
    ...
    it('should show the button for the simple_geom type', () => {
  ...
  describe('._initBinds', () => {
    ...
    it('should fetch queryGeometryModel if shouldFetch is true', () => {
      var queryGeometryModel = this.view._getQueryGeometryModel();
      jest.Ajax.stubRequest(/sql/)
    ...
    it('should call _renderContextButtons and set default style properties if there is a change in simple_geom to a valid value', () => {
      var queryGeometryModel = this.view._getQueryGeometryModel();
      queryGeometryModel.set('status', 'unfetched');
      queryGeometryModel.set('simple_geom', 'point');
      queryGeometryModel.set('query', 'SELECT TOP 10 FROM fake_table;');
    ...
    it('should call _renderContextButtons and not set default style properties if there is a change in simple_geom to nothing', () => {
      var queryGeometryModel = this.view._getQueryGeometryModel();
      queryGeometryModel.set('status', 'unfetched');
      queryGeometryModel.set('simple_geom', 'point');
      queryGeometryModel.set('query', 'SELECT TOP 10 FROM fake_table;');
