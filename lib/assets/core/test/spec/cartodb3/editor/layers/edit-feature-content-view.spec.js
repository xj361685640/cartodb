...
describe('editor/layers/edit-feature-content-view', () => {
  beforeEach(() => {
    jest.Ajax.install();
    jest.Ajax.stubRequest(new RegExp('^http(s)?://.*tables.*'))
    ...
    this.mapStackLayoutModel = jest.createSpyObj('stackLayoutModel', ['prevStep', 'nextStep', 'goToStep']);
    this.modals = new ModalsService();

    this.rowCollection = {
      resetFetch: jest.createSpy('resetFetch')
    };
  ...
  describe('when feature already exists', () => {
    ...
    it('should not add row', () => {
      expect(view._addRow).not.toHaveBeenCalled();
      expect(view._renderInfo).toHaveBeenCalled();
    });
  });

  describe('._onDestroyFeatureSuccess', () => {
    it('should call `_onUpdateFeature`', () => {
  ...
  describe('._onSaveFeatureSuccess', () => {
    it('should call `_onUpdateFeature`', () => {
      spyOn(this.view, '_onUpdateFeature').and.callThrough();
  ...
  describe('._onUpdateFeature', () => {
    it('should reset fetch if saving and current geometry model does not have value', () => {
    ...
    it('should not reset fetch if saving and current geometry model has value', () => {
    ...
    it('should reset fetch if destroying', () => {
      this.view._queryGeometryModel = new QueryGeometryModelMock();
      spyOn(this.view._queryGeometryModel, 'resetFetch');
