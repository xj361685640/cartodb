...
describe('editor/layers/layer-content-view/analyses/analysis-form-view', () => {
  beforeEach(() => {
  ...
  it('should render with template and data from form model', () => {
    expect(this.view.$el.html()).toContain('form');
    expect(this.view.$el.html()).toContain('data-fields="radius"');
  });

  it('should not validate when view is rendered intially', () => {
    expect(camshaftReference.validate).not.toHaveBeenCalled();
  });

  describe('when form changes with erroneous data', () => {
    beforeEach(() => {
    ...
    it('should update form model anyway', () => {
    ...
    it('should set form validation errors on the model', function (done) {
      ...
      setTimeout(() => {
        ...
        expect(self.formModel.setFormValidationErrors.calls.argsFor(1)[0]).toEqual({radius: jest.any(String)});
        done();
      }, 0);
    });
    ...
    describe('when validation passes', () => {
      ...
      it('should remove form validation errors', () => {
  ...
  describe('_onChangeAnalysisFormView and _showAnalysisFormErrors', () => {
    ...
    it('_showAnalysisFormErrors should not commit the form if the current form is different from the one who called it', () => {
