...
describe('editor/layers/layer-content-views/analyses/analysis-form-models/area-of-influence-form-model', () => {
  beforeEach(() => {
    this._configModel = new ConfigModel({base_url: '/u/pepe'});
    this._configModel.dataServiceEnabled = () => {
      return true;
    };
  ...
  it('should not have disabled types', () => {
  ...
  describe('when model is created with custom distance and radius', () => {
    beforeEach(() => {
      this.a1.set({
        radius: 2000,
        distance: 'kilometers'
    ...
    it('should convert the radius to the distance scale', () => {
      expect(this.model.get('radius')).toEqual(2);
      expect(this.model.get('distance')).toEqual('kilometers');
    });
  });

  describe('when source is not valid for other type', () => {
    beforeEach(() => {
    ...
    it('should disable type since source is not valid input for the other type', () => {
      var typeOptions = this.model.schema.type.options.map(function (d) {
        return _.pick(d, 'val', 'disabled');
      });
      expect(typeOptions).toEqual([
  ...
  describe('when type is changed', () => {
    describe('for a new model', () => {
      ...
      it('should replace the attributes but maintain persisted state', () => {
        expect(this.model.attributes).toEqual({
          id: 'a1',
          source: 'a0',
          type: 'trade-area',
    ...
    describe('for an persisted model', () => {
      beforeEach(() => {
        this.prevSchema = this.model.schema;
  ...
  describe('.getTemplateData', () => {
    it('should return the default data for given type', () => {
      var bufferTemplateData = this.model.getTemplateData();
      expect(bufferTemplateData).toEqual({
        parametersDataFields: jest.any(String)
      });
  ...
  describe('.updateNodeDefinition', () => {
    ...
    it('should replace the attributes on the given node model but maintain values not visible/editable in the form', () => {
    ...
    it('should convert string values to integers before setting the attrs on model', () => {
      this.model.set({
        type: 'trade-area',
        dissolved: 'true',
        isolines: '3',
    ...
    it('should convert radius to meters according to the selected distance', () => {
      this.model.set({
        distance: 'kilometers',
        radius: 3
      });
