...
describe('editor/style/style-manager', () => {
  beforeEach(() => {
    jest.Ajax.install();
  ...
  describe('track changes', () => {
    ...
    it('should update the layer definition model based on previous custom css', () => {
      // remove binding to onChange so the style changes are not sent to cartodb.js layers (which are not created in the test suite)
      this.collection.off('change', this.collection._onChange, this.collection);
    ...
    it('should update sql when style changes', () => {
      // remove binding to onChange so the style changes are not sent to cartodb.js layers (which are not created in the test suite)
      this.collection.off('change', this.collection._onChange, this.collection);
      this.layerDefModel.styleModel.set({
        type: 'hexabins',
    ...
    it('should update layer type when style is animation', () => {
      // remove binding to onChange so the style changes are not sent to cartodb.js layers (which are not created in the test suite)
      this.collection.off('change', this.collection._onChange, this.collection);
      this.layerDefModel.styleModel.set({
        type: 'animation',
    ...
    it('should change alphamarker url based on asset host', () => {
      // remove binding to onChange so the style changes are not sent to cartodb.js layers (which are not created in the test suite)
      this.collection.off('change', this.collection._onChange, this.collection);
      this.layerDefModel.styleModel.set({
        type: 'heatmap',
    ...
    it('should track style changes on add', () => {
      this.collection.add({
        id: 'l-101',
        order: 2,
        kind: 'carto',
    ...
    it('should track changes on autoStyle', () => {
