...
describe('components/form-components/editors/fill/input-color/input-categories/categories-list/categories-list-item-view', () => {
  beforeEach(() => {
  ...
  describe('_onClick', () => {
    it('should set js-assetPicker class in model when image is clicked', () => {
      this.view.$('.js-image-container').click();
      expect(this.model.get('selectedClass')).toEqual(['js-assetPicker']);
    });

    it('should set js-assetPicker class in model when IMG label is clicked', () => {
    ...
    it('should not set js-assetPicker class in model when rest of the item is clicked', () => {
      this.model.unset('image');
      this.view.render();
      this.view.$el.click();
