var InputColorPickerView = require('../../../../../../../../javascripts/cartodb3/components/form-components/editors/fill/input-color/input-color-picker/input-color-picker-view');

describe('components/form-components/editors/fill/input-color/input-color-picker/input-color-picker-view', () => {
  beforeEach(() => {
    this.view = new InputColorPickerView({ ramp: [ 303 ], opacity: 0.81 });
  });

  describe('initialization', () => {
    it('should throw Error if no ramp provided', () => {
      var wrapper = () => {
        new InputColorPickerView({}); // eslint-disable-line no-new
      };

      expect(wrapper).toThrowError('ramp is required');
    });

    it('should throw Error if no opacity provided', () => {
      var wrapper = () => {
        new InputColorPickerView({ ramp: [] }); // eslint-disable-line no-new
      };

      expect(wrapper).toThrowError('opacity is required');
    });

    it('should create proper model and bindings', () => {
      var view = new InputColorPickerView({ ramp: [ 303 ], opacity: 0.81 });

      expect(view.model.get('ramp')).toEqual([ 303 ]);
      expect(view.model.get('opacity')).toEqual(0.81);
    });

    it('should have bindings for changes in model', () => {
      var view = new InputColorPickerView({ ramp: [ 303 ], opacity: 0.81 });
      expect(view.model._events['change:index'][0].callback).toEqual(jest.anything());
      expect(view.model._events['change:opacity'][0].callback).toEqual(jest.anything());
    });
  });

  describe('._onOpacityChanged', () => {
    it('should trigger change:opacity when opacity changes', () => {
      var view = new InputColorPickerView({ ramp: [ 303 ], opacity: 0.81 });
      var opacity = 0;
      function onOpacityChanged (value) {
        opacity = value;
      }
      view.bind('change:opacity', onOpacityChanged);

      view.model.set('opacity', 0.5);

      expect(opacity).toBe(0.5);
    });
  });

  it('should not have leaks', () => {
    expect(this.view).toHaveNoLeaks();
  });
});
