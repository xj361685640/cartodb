...
describe('stack-layout/view', () => {
  ...
  it('should generate a view from the beginning', () => {
    expect(this.view._getCurrentView()).not.toBeUndefined();
  });

  describe('onChangePosition', () => {
    it('should listen a change position from the view model', () => {
      var args = this.viewModel.bind.calls.argsFor(0);
      expect(args[0]).toEqual('positionChanged');
    });

    it('should remove previous view when there is a position change', () => {
    ...
    it('should generate a new view when there is a position change', () => {
    ...
    it('should pass all arguments to the new view', () => {
      spyOn(this.view, '_genNewStackView').and.callThrough();
