var Backbone = require('backbone');
var FormView = require('../../../../../../javascripts/cartodb3/components/modals/dataset-metadata/form/form-view');

describe('components/modals/dataset-metadata/form-view', () => {
  beforeEach(() => {
    this.view = new FormView({
      visDefinitionModel: new Backbone.Model(),
      visMetadataModel: new Backbone.Model(),
      configModel: new Backbone.Model(),
      isLocked: false
    });
    this.view.model = {};
  });

  it('._commitView should call _sanitizeName', () => {
    spyOn(this.view, '_sanitizeName');
    var name = 'something';
    var view = {
      model: {
        changed: {name: name},
        get: () => {
          return name;
        }
      },
      commit: () => {}
    };

    this.view._commitView(view);

    expect(this.view._sanitizeName).toHaveBeenCalledWith(name);
  });

  it('._sanitizeName should sanitize input and set metadata model name', () => {
    this.view._sanitizeName('<script>');
    expect(this.view._visMetadataModel.get('name')).toEqual('');

    this.view._sanitizeName('something');
    expect(this.view._visMetadataModel.get('name')).toEqual('something');

    this.view._sanitizeName(null);
    expect(this.view._visMetadataModel.get('name')).toEqual('');
  });
});
