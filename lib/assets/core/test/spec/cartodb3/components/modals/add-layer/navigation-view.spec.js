...
describe('components/modals/add-layer/navigation-view', () => {
  beforeEach(() => {
    jest.Ajax.install();
  ...
  // it('should not render several links when create dialog type is dataset', () => {
  ...
  it('should render when listing state changes', () => {
    this.view._initBinds();
    var args = this.createModel.bind.calls.argsFor(0);
    expect(args[0]).toEqual('change:listing');
  });

  it('should create from scratch when create empty button is clicked', () => {
  ...
  it('should disable connect-dataset option when user can\'t create more datasets', () => {
  ...
  it('should disable create empty dataset option when user can\'t create more datasets', () => {
    spyOn(this.createModel, 'createFromScratch');
    this.createModel.set('listing', 'datasets');
    this.userModel.set('remaining_byte_quota', 0);
    this.view.render();
  ...
  it('should change router model and listing model when a link is clicked', () => {
    this.view.render();
