...
describe('components/modals/add-basemap/nasa/nasa-model', () => {
  beforeEach(() => {
  ...
  describe('when model changes', () => {
    ...
    it('should set layer', () => {
      expect(this.model.get('layer')).toEqual(jest.any(Object));
    });

    it('should set layer with expected params', () => {
      var layer = this.model.get('layer');
      expect(layer.get('urlTemplate')).toMatch('/2015-05-25/.*\{x\}');
      expect(layer.get('attribution')).toEqual(jest.any(String));
      expect(layer.get('maxZoom')).toEqual(jest.any(Number));
      expect(layer.get('minZoom')).toEqual(jest.any(Number));
      expect(layer.get('name')).toMatch('.* 2015-05-25');
    });
  });

  describe('.hasAlreadyAddedLayer', () => {
    beforeEach(() => {
    ...
    it('should return true if layer has already been added', () => {
