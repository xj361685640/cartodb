...
describe('components/modals/add-widgets/create-tuples-items', () => {
  beforeEach(() => {
    ...
    a1.queryGeometryModel.hasValue = () => {
      return true;
    };

    b1.queryGeometryModel.hasValue = () => {
      return true;
    };
  ...
  it('should return an object', () => {
    expect(this.tuplesItems).toEqual(jest.any(Object));
  });

  it('should contain an item for each column name+type match', () => {
    expect(this.tuplesItems['county_name-string'].length).toEqual(1);
    expect(this.tuplesItems['pop_count-number'].length).toEqual(2);
  });

  it('should not add items for blacklisted columns', () => {
  ...
  it('should not contain a tuple when no geometry data', () => {
    var b1 = this.analysisDefinitionNodesCollection.get('b1');
    b1.queryGeometryModel.hasValue = () => {
      return false;
    };
  ...
  it('should contain a tuple of column+layer definition in each item', () => {
    var tuples = this.tuplesItems['pop_count-number'];
  ...
  it('should skip nodes that does not have an owning layer', () => {
