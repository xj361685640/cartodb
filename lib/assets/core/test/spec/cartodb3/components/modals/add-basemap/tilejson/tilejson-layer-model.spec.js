...
describe('components/modals/add-basemap/tilejson/tilejson-layer-model', () => {
  beforeEach(() => {
  ...
  it('should have a custom URL to get JSON result', () => {
    expect(this.model.url()).toEqual('/tile.json');
  });

  describe('.newTileLayer', () => {
    describe('when have fetched model successfully', () => {
      beforeEach(() => {
        this.model.set(this.attrsExFromToTileJSONSpec);
      });

      it('should return a new tile layer', () => {
        expect(this.model.newTileLayer() instanceof CustomBaselayerModel).toBeTruthy();
      });

      it('should return model with expected attrs', () => {
      ...
      it('should set the TMS option based on the value of scheme', () => {
