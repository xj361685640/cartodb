...
describe('components/modals/add-basemap/nasa/nasa-view', () => {
  beforeEach(() => {
  ...
  it('should render day option as pre-selected', () => {
  ...
  it('should render date of yesterday as start date', () => {
    expect(this.innerHTML()).toMatch(/\d{4}-\d{2}-\d{2}/);
  });

  describe('when changing from day to night', () => {
    beforeEach(() => {
      this.view.$('.js-night').click();
    });

    it('should disable date picker', () => {
      expect(this.view.$('.DatePicker .DatePicker-dates').hasClass('is-disabled')).toBeTruthy();
    });

    describe('when changing back to day', () => {
      beforeEach(() => {
        this.view.$('.js-day').click();
      });

      it('should enable date picker again', () => {
